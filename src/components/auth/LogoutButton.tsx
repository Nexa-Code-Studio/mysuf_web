...
export default function LogoutButton({ className = "", variant = "solid" }: { className?: string; variant?: "solid" | "outline" }) {
  ...
  return (
    <button
      ...
      className={
        variant === "solid"
          ? `flex items-center justify-center rounded-xl bg-pertamina-red px-3 py-2.5 text-xs font-bold text-white transition hover:bg-[#c4142e] active:scale-[0.98] shadow-sm ${className}`
